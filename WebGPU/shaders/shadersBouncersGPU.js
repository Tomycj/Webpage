...
export function renderShader( N=10000, colorA = [1, 1, 1, 1], colorB = [1, 1, 1, 1] ) { return /*wgsl*/`
...
fn fragmentMain(input: FragInput) -> @location(0) vec4f {
    ...
    let nidx = f32( input.idx / ${N} ); // normalized particle index
    ...
    //return vec4f( nidx, 1 - nidx, (1 - nidx)/3, 1 );

    let colorA = vec4f(${colorA});
    let colorB = vec4f(${colorB});

    return mix(colorA, colorB, nidx);
